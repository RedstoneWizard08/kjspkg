use crate::{
    ...
    HttpResult, NewPackageVersion, PackageAuthor, PackageVersion,
};
use anyhow::anyhow;
use axum::{
    body::Body,
...
pub struct PartialPackageVersion {
    ...
    #[serde(default)]
    pub kubejs_versions: Option<String>,
}
...
#[utoipa::path(
    ...
    params(
        ("id" = String, Query, description = "The package ID whose versions we are looking for."),
    ),
)]
#[debug_handler]
...
#[utoipa::path(
    ...
    params(
        ("id" = String, Query, description = "The package that this version is for."),
        ("version" = String, Query, description = "The version ID/name/number."),
    ),
)]
#[debug_handler]
...
pub async fn create_handler(
    ...
    while let Ok(Some(field)) = data.next_field().await {
            ...
            .ok_or(anyhow!("Could not find a name for a field!"))?
        {
            ...
            "kubejs_versions" => kubejs_versions = Some(field.text().await?),
    ...
    if kubejs_versions.is_none() {
        Err(anyhow!("Missing field: 'kubejs_versions'"))?;
    }
    ...
    let kubejs_versions = kubejs_versions.unwrap();
...
pub async fn update_handler(
    ...
    let ver = update(package_versions::table)
        ...
        .set((
            ...
            package_versions::kubejs_versions
                .eq(data.kubejs_versions.unwrap_or(ver.kubejs_versions)),
        ))
